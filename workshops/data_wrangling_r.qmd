...
## {#title-slide data-menu-title="Data Wrangling in R" background-image="images/rainier.png" background-size="1950px 1200px" background-color="#4B2E83"}

[Data Wrangling in R]{.custom-title}

[CSSCR Workshop]{.custom-subtitle}

[`r format(Sys.Date(), '%d %B, %Y')`]{.custom-subtitle2}
...
# Importing and Exporting Data{.section-title background-color="#B7A57A"}
...
1. Using regular expressions to search for files by pattern-matching is extremely powerful, particularly if you're searching through large directories with many files. Learn more [here](https://r4ds.hadley.nz/regexps.html). 
...
# Manipulating and Summarizing Data{.section-title background-color="#B7A57A"}
...
# Merging Data {.section-title background-color="#B7A57A"}
...
# Tidying and Reshaping Data {.section-title background-color="#B7A57A"}
...
5. You can use `names_pattern` instead of `names_sep` to extract variables from more complicated naming scenarios once you've learned regular expressions in a few weeks. 
